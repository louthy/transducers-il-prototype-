...
public static class TResult
{
    ...
    public static TResult<S> Recursive<S, A>(S s, A value, Reducer<A, S> reduce) => 
        new TRecursive<S>(new TRecursiveReduce<A, S>(s, value, reduce));
...
public sealed record TContinue<A>(A Value) : TResult<A>
{
    ...
    public override TResult<S> Reduce<S>(TState state, S stateValue, Reducer<A, S> reducer) =>
        TResult.Recursive(stateValue, Value, reducer);
...
public sealed record TRecursiveReduce<A, S>(S StateValue, A Value, Reducer<A, S> Next) 
    : TRecursiveRunner<S>
{
    public override TResult<S> Run() =>
        Next.Run(StateValue, Value);
}
